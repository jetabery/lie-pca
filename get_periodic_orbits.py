...
def plot_orbit_and_approx(X, x_approx):
    plt.figure(figsize=(10, 6))
    plt.plot(X[:, 0], X[:, 1], label='Body 1 (actual)', color='red', linestyle='--')
    plt.plot(X[:, 2], X[:, 3], label='Body 2 (actual)', color='green', linestyle='--')
    plt.plot(X[:, 4], X[:, 5], label='Body 3 (actual)', color='blue', linestyle='--')
    plt.plot(x_approx[:, 0], x_approx[:, 1], label='Body 1 (Lie approx)', color='red', linewidth=4, alpha=0.3)
    plt.plot(x_approx[:, 2], x_approx[:, 3], label='Body 2 (Lie approx)', color='green', linewidth=4, alpha=0.3)
    plt.plot(x_approx[:, 4], x_approx[:, 5], label='Body 3 (Lie approx)', color='blue', linewidth=4, alpha=0.3)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Paths of the 3 Bodies')
    plt.legend()
